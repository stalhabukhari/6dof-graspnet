...
# for headless mode
RUN HOROVOD_GPU_OPERATIONS=NCCL LD_LIBRARY_PATH=/usr/local/cuda/lib64/stubs/:$LD_LIBRARY_PATH pip install horovod
...
# RUN pip install -r /env-setup/requirements-gui.txt
...
# RUN apt update && \
#     apt install -y --no-install-recommends libgl1-mesa-glx libgl1-mesa-dri libglu1-mesa mesa-utils
